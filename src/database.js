        ...
        async function integrityCheck () {
          ...
          function checkEntry (req, primitives, objects, arrays) {
            ...
            function isValue (value, key) {
              ...
              throw new Error(`Bad value ${value} provided for field ${key} in table ${tableName}. We expect null, a ${tablesModel[key].type}, an object, or an array of these types.`)
            }
            ...
            function isObject (value, key) {
              if (value !== null && isPrimitive(value)) throw new Error(`Bad value ${value} provided for field ${key} in table ${tableName}. We expect null, an object, or an array of these types.`)
              return true
            }
          ...
          try {
            ...
            if (request.set) {
              if (Array.isArray(request.set) || !(request instanceof Object)) throw new Error(`The 'set' instruction ${JSON.stringify(request.set)} provided in table ${tableName} is not a plain object. A plain object is required for 'set' instructions.`)
              ...
              if (wrongKey) throw new Error(`The value ${stringify(request.set[wrongKey])} for ${wrongKey} in table ${tableName} is of type ${toType(request.set[wrongKey])} but it was expected to be of type ${tablesModel[tableName][wrongKey].type}.`)
            }
            // Check that create instruction is acceptable
            if (request.create) {
              const wrongKey = wrongType(primitives, request, tablesModel[tableName])
              if (wrongKey) throw new Error(`The value ${stringify(request[wrongKey])} for ${wrongKey} in table ${tableName} is of type ${toType(request[wrongKey])} but it was expected to be of type ${tablesModel[tableName][wrongKey].type}.`)
            }
            ...
            if (unwantedInstruction) throw new Error(`Do not use 'add' or 'remove' instructions within ${unwantedInstruction} parameter in table ${tableName}. You should use the 'set' instruction instead.`)
            ...
            if (request.create || request.delete) {
              ...
              if (addOrRemove) {
                throw new Error(request.create ? `In create requests, you cannot have 'add' or 'remove' instructions in ${addOrRemove} in table ${tableName}. To add children, just write the constraints directly under ${addOrRemove}.`
            ...
            if (request.limit && !Number.isInteger(request.limit)) throw new Error(`'Limit' statements requires an integer within ${tableName}. We received: ${request.limit} instead.`)
            if (request.offset && !Number.isInteger(request.offset)) throw new Error(`'Offset' statements requires an integer within ${tableName}. We received: ${request.offset} instead.`)
            if (request.order) {
            // Ensure that order is an array of strings
              if (!Array.isArray(request.order)) throw new Error(`'order' statements requires an array of column names within ${tableName} request. We received: ${request.order} instead.`)
              ...
              if (unfoundColumn) throw new Error(`'order' statement requires an array of property names within ${tableName}, but ${unfoundColumn} doesn't belong to this table.`)
            }
